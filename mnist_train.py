...
parser.add_argument('--dist-url', default='tcp://172.18.146.204:23455', type=str,
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        print('\nRank {},Test set: Average loss: {:.4f}, Accuracy:({:.0f}%)\n'.format(
            args.rank, losses.avg, acc))

    return acc


def save_checkpoint(state, is_best, filename='./pth/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, './pth/model_best.pth.tar')
